...
int main(void)
{
  ...
  DBG("MCU and Peripherals has been initialised");

  if (tm1637_init(&tm1637, &htim9) != Tm1637_Ok) {
	  Error_Handler();
  }
  ...
  while (1)
  {
	  ...
	  if (now - last_print >= 1000) {
		  DBG("Tick %lu - count = %lu", now / 1000, tm1637.tick_count);
		  last_print = now;
	  }
